...
class BasicTestsSpecification extends ServerCoreSpecification implements CoreGroovyTrait {
    def setupSpec() {
        ...
        TestingOps.setupServerCoreDefault([
            "classpath:/com/ait/tooling/server/core/test/ApplicationContext.xml",
    ...
    def "Parse Lion JSON out"() {
        setup:
        def t = new NanoTimer()
        def r = resource('classpath:/com/ait/tooling/server/core/test/lion.json')
    ...
    def "Parse Tiger JSON"() {
        setup:
        def r = resource('classpath:/com/ait/tooling/server/core/test/tiger.json')
