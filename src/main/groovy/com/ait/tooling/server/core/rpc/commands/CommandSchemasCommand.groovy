...
@Authorized
@CompileStatic
public class CommandSchemasCommand extends CoreGroovyCommandSupport
{
    @Override
    public JSONObject execute(IJSONRequestContext context, JSONObject object) throws Exception
    {
        IJSONCommand command = getCommand(object['name'].toString())

        if (command)
        {
            AuthorizationResult auth = getAuthorizationProvider().isAuthorized(command, context.getUserPrincipals())
